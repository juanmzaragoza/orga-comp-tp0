	...
	if (status == SUCCESS) {
		...
		while (!feof(fp_in)) {
			...
			if (fscanf(fp_in, "%d", &num_rows) == 1) {
				...
				i = 0;
				while (i < num_rows * num_rows
						&& fscanf(fp_in, "%g", &element) == 1) {
					matrix1->array[i] = element;
					i++;
				}

				while (i >= (num_rows * num_rows)
						&& (i < num_rows * num_rows * 2)
						&& fscanf(fp_in, "%g", &element) == 1) {
	...
	for (i = 0; i < m1->rows; i++) {
		...
		for (j = 0; j < m2->cols; j++) {
			...
			for (k = 0; k < m1->cols; k++) {/* recorro la fila de m1 y la columna de m2 */
				m_res->array[j + i * m2->cols] += m1->array[k + i * m1->cols]
						* m2->array[j + k * m2->cols];
			}
